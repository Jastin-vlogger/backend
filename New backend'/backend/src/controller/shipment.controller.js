...
exports.getShipmentSummary = async (req, res) => {
  try {
    ...
    const logisticsPendingCount = containers.filter((container) =>
      !!container.actual && !String(container.actual.BLNo || '').trim()
    ).length;
    const rolePending = {
      role: normalizedRole || 'Unknown',
      label: normalizedRole === 'Logistic' ? 'Pending BL Details' : 'Pending For Your Role',
